...
public class Board
{
    ...
    private static readonly NormalItem.eNormalType[] NORMAL_TYPES = Enum.GetValues(typeof(NormalItem.eNormalType)).Cast<NormalItem.eNormalType>().ToArray();
    ...
    public Board(Transform transform, GameSettings gameSettings)
    {
        ...
        m_bottomCollector = new BottomCollector(GetBottomCells());
    }
    ...
    internal IEnumerable<Cell> GetTopCells()
    {
        for (int x = 0; x < boardSizeX; x++)
        {
    ...
    internal IEnumerable<Cell> GetBottomCells()
    {
        for (int x = 0; x < boardSizeX; x++)
        {
            yield return m_cells[x, 0];
        }
    }

    internal BottomCollector GetBottomCollector()
    {
        return m_bottomCollector;
    }

    internal bool AreTopCellsEmpty()
    {
        return GetTopCells().All(cell => cell.IsEmpty);
    }

    internal int CountTopItems()
    {
        return GetTopCells().Count(cell => !cell.IsEmpty);
    }

    internal IEnumerable<Cell> GetTopCells(NormalItem.eNormalType type)
    {
        return GetTopCells().Where(cell => cell.Item is NormalItem normal && normal.ItemType == type);
    }

    internal IEnumerable<Cell> GetNonEmptyTopCells()
    {
        return GetTopCells().Where(cell => !cell.IsEmpty);
    }

    internal Dictionary<NormalItem.eNormalType, List<Cell>> GetTopCellsGroupedByType()
    {
        return GetNonEmptyTopCells()
    ...
    internal void Fill()
    {
        List<Cell> topCells = GetTopCells().ToList();
        int totalCells = topCells.Count;

        if (totalCells % 3 != 0)
        {
            Debug.LogWarning("[Board] Số ô khu vực chơi không chia hết cho 3. Một số ô sẽ để trống.");
        }

        List<NormalItem.eNormalType> typePool = BuildInitialTypePool(totalCells);

        Shuffle(typePool);

        for (int i = 0; i < topCells.Count; i++)
        {
            Cell cell = topCells[i];
            cell.Free();
    ...
    private List<NormalItem.eNormalType> BuildInitialTypePool(int totalCells)
    {
        int groups = totalCells / 3;
        int uniqueTypes = Mathf.Min(groups, NORMAL_TYPES.Length);

        List<NormalItem.eNormalType> shuffledTypes = NORMAL_TYPES.OrderBy(x => UnityEngine.Random.value).ToList();
        List<NormalItem.eNormalType> pool = new List<NormalItem.eNormalType>(groups * 3);

        for (int i = 0; i < groups; i++)
        {
            NormalItem.eNormalType type = shuffledTypes[i % uniqueTypes];
            pool.Add(type);
            pool.Add(type);
            pool.Add(type);
        }
